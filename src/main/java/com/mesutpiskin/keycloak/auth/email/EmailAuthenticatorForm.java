...
public class EmailAuthenticatorForm implements Authenticator {
    ...
    private void generateAndSendEmailCode(AuthenticationFlowContext context) {
        ...
        if (context.getAuthenticationSession().getAuthNote(EMAIL_CODE) != null) {
            ...
            log.warnf("Skip sending email block");
            return;
        }

        int emailCode = ThreadLocalRandom.current().nextInt(999999);
    ...
    private void sendEmailWithCode(RealmModel realm, UserModel user, String code) {
        ...
        mailBodyAttributes.put("secret", code);
        mailBodyAttributes.put("realm", realmName);
        
        // System.out.println("mailBodyAttributes");
        // System.out.println(code);
        
        List<Object> subjectParams = List.of(realmName);
        // System.out.println("subjectParams");
        // System.out.println(subjectParams);
        try {
            ...
            emailProvider.send("Sampark-"+realmName+" 2FA Authentication", subjectParams, "code-email.ftl", mailBodyAttributes);
